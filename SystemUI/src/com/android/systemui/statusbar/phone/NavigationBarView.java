        ...
        public void handleMessage(Message m) {
            switch (m.what) {
                case MSG_CHECK_INVALID_LAYOUT:
                    ...
                    break;
                case MSG_TESTCENTER_TRIGGER_TIMEOUT:
                    Log.d("liujg6", "===Lenovo Test Mode trigger event timeout, cancel trigger!===");
                    mAreaClickedTimes = 0;
                    break;
            }
        }
    ...
    void notifyKeyEvent(KeyEvent event) {
        ...
        if(keyCode == KeyEvent.KEYCODE_VOLUME_UP) {
            if(action == KeyEvent.ACTION_DOWN) {
                mIsVolUpPressed = true;
                Log.d("liujg6", "volume up key was pressed");
            }
            if(action == KeyEvent.ACTION_UP) {
                mIsVolUpPressed = false;
                Log.d("liujg6", "volume up key was released");
            }
        }
    }

    private boolean mIsVolUpPressed = false;
    private int mAreaClickedTimes = 0;
    final static int MSG_TESTCENTER_TRIGGER_TIMEOUT = 8787;
    final static int TESTCENTER_TRIGGER_TIMEOUT_IN_MS = 3000;
    @Override
    public boolean onTouchEvent(MotionEvent event) {
        final int action = event.getAction();
        if(action == MotionEvent.ACTION_UP){
	        int dpi = mContext.getResources().getDisplayMetrics().densityDpi;
	        float scale = dpi * DisplayMetrics.DENSITY_DEFAULT_SCALE;
	        float xlimit = 50 * scale;
	        Log.d("liujg6", "get touch UP event on navigation bar, x = " + event.getX() + ", y = " + event.getY() + 
	               ", dpi = " + dpi + ", scale = " + scale + ", xlimit = " + xlimit);
	        int x = (int)event.getX();
	        int y = (int)event.getY();
	        if((mIsVolUpPressed == true) && (x <= this.getLeft() + xlimit)) {
	            if(mAreaClickedTimes == 0){
	                mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_TESTCENTER_TRIGGER_TIMEOUT, 0, 0), TESTCENTER_TRIGGER_TIMEOUT_IN_MS);
	            }
		        mAreaClickedTimes ++;
	        } else {
		        mAreaClickedTimes = 0;
		        mHandler.removeMessages(MSG_TESTCENTER_TRIGGER_TIMEOUT);
	        }
	        if(mAreaClickedTimes == 4 && (mIsVolUpPressed == true)){
		        android.widget.Toast.makeText(mContext,R.string.launch_testcenter, android.widget.Toast.LENGTH_LONG).show();
		        mAreaClickedTimes = 0;
		        mHandler.removeMessages(MSG_TESTCENTER_TRIGGER_TIMEOUT);
	            Log.d("liujg6", "===get Lenovo Test Mode trigger event (4 constinous click on the left area of navigation bar)===");
	            Intent intent= new Intent("com.lenovo.testcenter.action.ENABLE_TESTCENTER");
	            mContext.sendOrderedBroadcast(intent, null);
	            /*try {
	                Intent intent1= new Intent();
	                intent1.setComponent(new ComponentName("com.lenovo.testcenter", "com.lenovo.testcenter.MainActivity"));
