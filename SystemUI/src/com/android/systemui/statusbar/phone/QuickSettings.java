...
import com.android.systemui.statusbar.phone.LenovoExtQuickSettingsModel.AudioProfileState;
import com.android.systemui.statusbar.phone.LenovoExtQuickSettingsModel.ScreenOffTimeoutState;
import com.android.systemui.statusbar.phone.QuickSettingsModel.ActivityState;
import com.android.systemui.statusbar.phone.QuickSettingsModel.BluetoothState;
import com.android.systemui.statusbar.phone.QuickSettingsConnectionModel;
...
class QuickSettings {
    ...
    private static QuickSettingsTileView smartSideBarTile;
    private Context mContext;
    private PanelBar mBar;
    private LenovoExtQuickSettingsModel mModel;
    ...
    public QuickSettings(Context context, QuickSettingsContainerView container) {
        ...
        mModel = new LenovoExtQuickSettingsModel(context);
    ...
    private void setupQuickSettings() {
        ...
        // add by xutg1, BLADEFHD-550, BLADEFHD-511hide user title in system ui for product has ideafriend
        if (!LenovoFeatureOption.LENOVO_IDEAFRIEND_SUPPORT) {
            queryForUserInformation();
            mTilesSetUp = true;
        }
        // // add end, BLADEFHD-550
    ...
    private void addSystemTiles(ViewGroup parent, LayoutInflater inflater) {
        ...
        if (mModel.deviceHasMobileData()) {
            ...
            mModel.addRSSITile(rssiTile, new NetworkActivityCallback() {
                ...
                public void refreshView(QuickSettingsTileView view, State state) {
                    ...
                    /*Begin,Lenovo-sw piyy1 add 2013-12-26, modify for add apn */
                    /*Original
                    setText(state.label);
                    */
                    String spn = state.label;

                    String[] spnItems = mContext.getResources().getStringArray(R.array.origin_carrier_names);
                    String[] spnValues = mContext.getResources().getStringArray(R.array.origin_carrier_values);
                    int i;
                    String spnItem = null;
                    String spnValue = null;

                    for(i = 0; i < spnItems.length; i++){
                        spnItem = spnItems[i].toString();
                        spnValue = spnValues[i].toString();
                        if(spnItem.equalsIgnoreCase(spn)){
                            spn = spnValue;
                        }
                    }

                    if(!spnValue.equalsIgnoreCase(spn)){
                        String[] spnItems_2g = mContext.getResources().getStringArray(R.array.origin_carrier_2g_names);
                        String[] spnValues_2g = mContext.getResources().getStringArray(R.array.origin_carrier_2g_values);
                        int j;

                        for(j = 0; j < spnItems_2g.length; j++){
                            String spnItem_2g = spnItems_2g[j].toString();
                            String spnValue_2g = spnValues_2g[j].toString();
                            if(spnItem_2g.equalsIgnoreCase(spn)){
                                spn = spnValue_2g;
                            }
                        }
                    }
                    ...
                    tv.setText(spn);
                    /*End,Lenovo-sw piyy1 add 2013-12-26, modify for add apn */
            ...
            final QuickSettingsBasicTile mobileDataTile = new QuickSettingsBasicTile(mContext);
            mModel.addMobileDataTile(mobileDataTile, new QuickSettingsModel.RefreshCallback() {
                @Override
                public void refreshView(QuickSettingsTileView unused, State state) {
                    mobileDataTile.setImageResource(state.iconId);
                    String mobileDataState = mContext.getString(
                            (state.enabled) ? R.string.accessibility_desc_on
                                    : R.string.accessibility_desc_off);
                    mobileDataTile.setContentDescription(
                            mContext.getString(R.string.accessibility_quick_settings_mobile_data,
                                    mobileDataState));
                    mobileDataTile.setText(state.label);
                }
            });
            parent.addView(mobileDataTile);
        }
        ...
        mModel.addBatteryTile(batteryTile, new QuickSettingsModel.RefreshCallback() {
            ...
            public void refreshView(QuickSettingsTileView unused, State state) {
                ...
                LevelListDrawable batteryLevels = (LevelListDrawable) r.getDrawable(R.drawable.qs_sys_battery);
                LevelListDrawable chargingBatteryLevels = (LevelListDrawable) r.getDrawable(R.drawable.qs_sys_battery_charging);
        ...
        // Screen Off Timeout
        final QuickSettingsBasicTile screenOffTimeoutTile = new QuickSettingsBasicTile(
                mContext);
        screenOffTimeoutTile.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
        ...
        mModel.addScreenOffTimeoutTile(screenOffTimeoutTile, new QuickSettingsModel.RefreshCallback() {
            @Override
            public void refreshView(QuickSettingsTileView unused,
                    State state) {
                ScreenOffTimeoutState timeoutState = (ScreenOffTimeoutState)state;
                screenOffTimeoutTile.setImageResource(timeoutState.iconId);
                screenOffTimeoutTile.setContentDescription(mContext
                        .getString(
                                R.string.accessibility_quick_settings_screen_off_timeout,
                                timeoutState.description));
                screenOffTimeoutTile.setText(state.label);
            }
        });
        parent.addView(screenOffTimeoutTile);

        /*added by weiyl2 2014-3-19*/
        final QuickSettingsTileView smartSideBarTile = (QuickSettingsTileView)
                inflater.inflate(R.layout.quick_settings_tile, parent, false);
        smartSideBarTile.setContent(R.layout.quick_settings_tile_side_bar, inflater);
        parent.addView(smartSideBarTile);
        mQuickSettingsConnectionModel.sendNotificationForSmartBar(QuickSettingsConnectionModel.getSmartSideBarScene());
        mQuickSettingsConnectionModel.addSmartSideBarTile(smartSideBarTile);

        //Audio Profile
        final QuickSettingsBasicTile audioProfileTile = new QuickSettingsBasicTile(mContext);
        audioProfileTile.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
        ...
        mModel.addAudioProfileTile(audioProfileTile, new QuickSettingsModel.RefreshCallback() {
            @Override
            public void refreshView(QuickSettingsTileView unused, State state) {
                AudioProfileState audioProfileState = (AudioProfileState)state;
                audioProfileTile.setImageResource(audioProfileState.iconId);
                audioProfileTile.setContentDescription(mContext.getString(
                                R.string.accessibility_quick_settings_audio_profile,
                                audioProfileState.description));
                audioProfileTile.setText(audioProfileState.label);
            }
        });
        parent.addView(audioProfileTile);

        //Dolby
        final QuickSettingsBasicTile dolbyTile = new QuickSettingsBasicTile(mContext);
        dolbyTile.setOnLongClickListener(new View.OnLongClickListener() {
            @Override
            public boolean onLongClick(View v) {
                startSettingsActivity("com.dolby.LAUNCH_DS_APP");
                return true;
            }
        });
        mModel.addDolbyTile(dolbyTile, new QuickSettingsModel.RefreshCallback() {
            @Override
            public void refreshView(QuickSettingsTileView view, State state) {
                dolbyTile.setImageResource(state.iconId);
                String dolbyState = mContext
                        .getString((state.enabled) ? R.string.accessibility_desc_on
                                : R.string.accessibility_desc_off);
                dolbyTile.setContentDescription(mContext
                        .getString(R.string.accessibility_quick_settings_dolby, dolbyState));
                dolbyTile.setText(state.label);
            }
        });
        parent.addView(dolbyTile);
    }
    ...
    private void addTemporaryTiles(final ViewGroup parent, final LayoutInflater inflater) {
        ...
        alarmTile.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // lenovo-sw, [BLADEFHD-1777], Alarm tile click issue, start
                if (LenovoFeatureOption.LENOVO_LENOVOCLOCK_SUPPORT) {
                    Intent intent = new Intent();
                    String package_name = "com.android.deskclock";
                    String class_name = "com.android.deskclock.DeskClock";
                    intent.setComponent(new ComponentName(
                            package_name,
                            class_name));
                    startSettingsActivity(intent);
                ...
                // lenovo-sw, [BLADEFHD-1777], Alarm tile click issue, end
            }
        });
        ...
        final QuickSettingsBasicTile bugreportTile
                = new QuickSettingsBasicTile(mContext);
        bugreportTile.setImageResource(R.drawable.ic_qs_adb_am);
    ...
    private void showBugreportDialog() {
        final AlertDialog.Builder builder = new AlertDialog.Builder(mContext, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT); //BLADEFHDKK-143
    ...
    private abstract static class NetworkActivityCallback
        ...
        private void setVisibility(View view, boolean visible) {
            ...
            if ((view != null) && (view.getAlpha() != newAlpha)) {
                view.animate()
