...
public class UsbDebuggingActivity extends AlertActivity
    ...
    public void onCreate(Bundle icicle) {
      ...
      //BLADEFHD-1200, lenovo UI update
      if (this.isIdeaUITheme()) {
        LayoutInflater inflater = LayoutInflater.from(ap.mContext);
        View usbView = inflater.inflate(R.layout.usb_allow_debug_confirm, null);
        if (usbView != null) {
            TextView msgTextView = (TextView)usbView.findViewById(R.id.usb_debug_msg);
            msgTextView.setText(getString(R.string.usb_debugging_message, fingerprints));
            mAlwaysAllow = (CheckBox)usbView.findViewById(com.android.internal.R.id.alwaysUse);
            mAlwaysAllow.setText(getString(R.string.usb_debugging_always));
            ap.mView = usbView;
        } else {
            Log.d(TAG, "usbView is null");
            finish();
            return;
        }
      } else {
