...
/*Begin, lenovo-sw caijh, add dialog theme, 2012.8.30*/
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    /*Begin,Lenovo-sw piyy1 add 2014-01-17, add for support baidu tv rotation */
    private boolean isAutoRotationEnabled(){
        String isChecked = SystemProperties.get("persist.sys.smart_rotate", "0");
        return "1".equals(isChecked)?true:false;
    }
    /*End,Lenovo-sw piyy1 add 2014-01-17, add for support baidu tv rotation */
    ...
    // When cover is on, delay a short time before going to sleep
    private static final long COVERED_GOTOSLEEP_DELAY_MILLIS = 300;
    ...
    private static final int MSG_COVERED_GOTOSLEEP = 5;
    boolean mWifiDisplayConnected = false;
    int     mWifiDisplayUIBCRotation = -1;

    // use to adjust brightness value
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_DISPATCH_MEDIA_KEY_REPEAT_WITH_WAKE_LOCK:
                    ...
                    break;
                 case MSG_COVERED_GOTOSLEEP:
                    Slog.d(TAG, "Cover on, go to sleep after delay");
    ...
    boolean needSensorRunningLp() {
		...
		/*Begin,Lenovo-sw xubo6 modify to support Blade game mode 2013-07-25 */ 
		||(mCurrentAppOrientation == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT
		   && isAutoRotationEnabled())
		||(mCurrentAppOrientation == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE
		   && isAutoRotationEnabled())
		/*End,Lenovo-sw xubo6 modify to support Blade game mode 2013-07-25 */
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        // register for cover events
        IntentFilter coverFilter = new IntentFilter();
        coverFilter.addAction(Intent.ACTION_HALL_EVENT);
        Intent coverIntent = context.registerReceiver(mCoverReceiver, coverFilter);
        if (coverIntent != null) {
            // Retrieve current sticky cover event broadcast.
            mCoverMode = coverIntent.getIntExtra(Intent.EXTRA_HALL_STATE,
                    Intent.EXTRA_HALL_STATE_FAR);
        }
...
//dingej1 2014.06.26 begin. BLADEFHKDD-744 port from k6_kk product
                    //dcf.top = mStableTop; //this fix dialog cut off by zhaoll8
                    dcf.top = mDockTop;
//dingej1 end.
                }
        ...
        if (mStatusBar != null) {
            ...
            if (mForceStatusBar || mForceStatusBarFromKeyguard) {
                ...
                if(IdeaUI.isPlatfromSupportIdeaUI()) {
                    IdeaUI.roundedCornerCover_setTopIsFullScreen(topIsFullscreen);
                }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        switch (keyCode) {
            ...
            // add by xutg1, add lenovo BT keyboard support
            case KeyEvent.KEYCODE_BRIGHTNESS_DOWN:
            case KeyEvent.KEYCODE_BRIGHTNESS_UP: {
                if (LenovoFeatureOption.LENOVO_BTKEYBOARD_COVER) {
                    if (down) {
                        setBrightness(keyCode);
                    }
                }
                break;
            }

            case KeyEvent.KEYCODE_SCREENLOCK_DOCK: {
                if (LenovoFeatureOption.LENOVO_BTKEYBOARD_COVER) {
                    if (down) {
                        if (!isScreenOn) {
                            mBroadcastWakeLock.acquire();
                            ...
                            if (mKeyguardDelegate != null) {
                                mKeyguardDelegate.onScreenTurnedOff(WindowManagerPolicy.OFF_BECAUSE_OF_USER);
                            }
                        }
                    }
                }
            ...
            case KeyEvent.KEYCODE_PRTSC: {
                if (LenovoFeatureOption.LENOVO_BTKEYBOARD_COVER) {
                    if (down && isScreenOn) {
                        prtscKeyAction(keyCode);
                    }
                }
                break;
            }
    ...
    // add by xutg1, add lenovo BT keyboard support
    public void setBrightness(int keyCode) {
        int mode = 0;
        int MINIMUM_BACKLIGHT = 20;  // class android.os.Power was deprecated, use the value of BRIGHTNESS_DIM directly
        int MAXIMUM_BACKLIGHT = 255; // class android.os.Power was deprecated, use the value of BRIGHTNESS_ON directly
        int value = MAXIMUM_BACKLIGHT;
        ...
        try {
            ...
            // change the mode to manual if it was automatic
            ...
            switch (keyCode) {
                case KeyEvent.KEYCODE_BRIGHTNESS_DOWN: {
                    value = value - ((int)(0.1 * MAXIMUM_BACKLIGHT));
                    break;
                }
                case KeyEvent.KEYCODE_BRIGHTNESS_UP: {
                    value = value + ((int)(0.1 * MAXIMUM_BACKLIGHT));
            ...
            } else if (value < MINIMUM_BACKLIGHT) {
    ...
    public void syncBrightness(int value) {
        final int value_to_set = value;

        try {
            mPower = IPowerManager.Stub.asInterface(ServiceManager.getService("power"));
            if (mPower != null) {
                if (mPower.isScreenOn()) {
                    mPower.setTemporaryScreenBrightnessSettingOverride(value_to_set);
                } else {
                    mPower.setTemporaryScreenBrightnessSettingOverride(-1);
                }
                AsyncTask.execute(new Runnable() {
                    public void run() {
                        Settings.System.putInt(mContext.getContentResolver(), Settings.System.SCREEN_BRIGHTNESS, value_to_set);
                        Intent intentBright = new Intent("ACTION_CHANGE_BRIGHTNESS");
                        intentBright.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY);
                        intentBright.putExtra("value", value_to_set);
                        mContext.sendBroadcast(intentBright);
                    }
                });
            }
        } catch (Exception ex) {
    ...
    public void prtscKeyAction(int keyCode) {
        if (keyCode == KeyEvent.KEYCODE_PRTSC)
    ...
    BroadcastReceiver mCoverReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            int curCoverMode = mCoverMode;
            Slog.d(TAG, "current cover mode: " + curCoverMode);

            if (Intent.ACTION_HALL_EVENT.equals(intent.getAction())) {
                mCoverMode = intent.getIntExtra(Intent.EXTRA_HALL_STATE,
                        Intent.EXTRA_HALL_STATE_FAR);
                Slog.d(TAG, "new cover mode: " + mCoverMode);
            }

            if (!isDeviceProvisioned() || (curCoverMode == mCoverMode)) {
                return;
            }

            applyCoverSwitchState();
        }
    };
    ...
    public int rotationForOrientationLw(int orientation, int lastRotation) {
		...
		case ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE:
			...
			return mLandscapeRotation;
		      }/* else isAutoRotationEnabled() == true,treat SCREEN_ORIENTATION_LANDSCAPE as SCREEN_ORIENTATION_SENSOR_LANDSCAPE
		/*End,Lenovo-sw xubo6 modify to support Blade game mode 2013-07-25 */
		...
		case ActivityInfo.SCREEN_ORIENTATION_PORTRAIT:
		     if (!isAutoRotationEnabled()){
	                ...
	                return mPortraitRotation;
		     }/* else isAutoRotationEnabled() == true,treat SCREEN_ORIENTATION_PORTRAIT as SCREEN_ORIENTATION_SENSOR_PORTRAIT
		/*End,Lenovo-sw xubo6 modify to support Blade game mode 2013-07-25 */
            ...
            @Override public void run() {
                if (mBootMsgDialog == null) {
                   /*Begin, lenovo-sw caijh, add dialog theme, 2012.8.30*/
                    ...
                    /*Lenovo-sw wuzb1 modify 2013-07-29 begin, set to ideaUI*/
                    mBootMsgDialog = new ProgressDialog(mContext, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT) {
                    /*Lenovo-sw wuzb1 modify 2013-07-29 end, set to ideaUI*/
                   /*End, lenovo-sw caijh, add dialog theme, 2012.8.30*/ 
    ...
    private void applyCoverSwitchState() {
        if (mHandler.hasMessages(MSG_COVERED_GOTOSLEEP)) {
            Slog.d(TAG, "Cover state changed, remove the pending gotosleep message");
            mHandler.removeMessages(MSG_COVERED_GOTOSLEEP);
        }

        if (mCoverMode == Intent.EXTRA_HALL_STATE_FAR) {
            Slog.d(TAG, "Cover off, turn screen on");
            screenTurningOn(new ScreenOnListener() {
                @Override
                public void onScreenOn() {
                    if (keyguardIsShowingTq()) {
                        if (!isKeyguardSecure()) {
                            Slog.d(TAG, "Cover off, dissmiss non secure keyguard");
                            dismissKeyguardLw();
                        }
                    } else {
                        mPowerManager.wakeUp(SystemClock.uptimeMillis());
       ...
       } else if (mCoverMode == Intent.EXTRA_HALL_STATE_NEAR) {
            // To handle repeatedly cover/uncover, deylay a short time before going to sleep.
            Slog.d(TAG, "Cover on, send gotosleep message delayed " + COVERED_GOTOSLEEP_DELAY_MILLIS + " ms");
            Message msg = mHandler.obtainMessage(MSG_COVERED_GOTOSLEEP);
            mHandler.sendMessageDelayed(msg, COVERED_GOTOSLEEP_DELAY_MILLIS);
        }
    }
