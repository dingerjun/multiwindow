...
class MountService extends IMountService.Stub
    ...
    private static final String                   EXTERNAL_SD1 = "/storage/sdcard0";
    ...
    class UmsEnableCallBack extends UnmountCallBack {
        ...
        void handleFinished() {
            ...
            // add by xutg1, add mass storage support
            if (LenovoFeatureOption.LENOVO_MASS_STORAGE_SUPPORT) {
                mUMSCount--;
                if (mUMSCount <= 0) {
                    new Thread() {
                        public void run() {
            ...
            switch (msg.what) {
                case H_UNMOUNT_PM_UPDATE: {
                    ...
                    if (DualSdHelper.isDualSdSupport()){
                        if (!mUpdatingStatus) {
                            mUpdateingBackMount = 0;
                        }
                    }
                    //if (!mUpdatingStatus) {
                    if (true){
                        if (DEBUG_UNMOUNT) Slog.i(TAG, "Updating external media status on PackageManager");
                        // sdcard1 update media status. oujf1
                        if (ucb.path.equals(EXTERNAL_SD1)) {
                            mUpdatingStatus = true;
                            if (DualSdHelper.isDualSdSupport()){
                                mUpdateingBackMount ++;
                                Slog.i(TAG, String.format("mUpdateingBackMount (%d)", mUpdateingBackMount));
                            }
                            mPms.updateExternalMediaStatus(false, true);
                        } else if (DualSdHelper.isDualSdSupport() && ucb.path.equals(EXTERNAL_SD2)) {
                            mUpdatingStatus = true;
                            mUpdateingBackMount ++;
                            Slog.i(TAG, String.format("mUpdateingBackMount (%d)", mUpdateingBackMount));
                            mPms.updateExternalMediaStatusEx(false, true, 2);
                        } else {
                            // add by xutg1, BLADEFHD-636, enable SD/OTG erase
                            mUpdatingStatus = true;
                            mUpdateingBackMount ++;
                            mPms.updateExternalMediaStatus(false, true);
                            // add end, BLADEFHD-636
                        }
                    }
                    break;                }
    ...
    private void updatePublicVolumeState(StorageVolume volume, String state) {
        ...
        // when not emulated.
        if (path.equals(EXTERNAL_SD1) || (DualSdHelper.isDualSdSupport() && path.equals(EXTERNAL_SD2))) {
        //if (volume.isPrimary() && !volume.isEmulated()) {
            if (Environment.MEDIA_UNMOUNTED.equals(state)) {
                if (path.equals(EXTERNAL_SD1)) {
                    mPms.updateExternalMediaStatus(false, false);
                } else if (DualSdHelper.isDualSdSupport() && path.equals(EXTERNAL_SD2)) {
                    mPms.updateExternalMediaStatusEx(false, false, 2);
                }
            ...
            } else if (DualSdHelper.isDualSdSupport() && Environment.MEDIA_MOUNTED.equals(state) && path.equals(EXTERNAL_SD2)) {
                mPms.updateExternalMediaStatusEx(true, false, 2);
            }
        }
    ...
    private int doUnmountVolume(String path, boolean force, boolean removeEncryption) {
        ...
        // sdcard1 update media status. mengfandong
        if (path.equals(EXTERNAL_SD1)) {
            //mPms.updateExternalMediaStatus(false, false);
        } else if (DualSdHelper.isDualSdSupport() && path.equals(EXTERNAL_SD2)) {
            //mPms.updateExternalMediaStatusEx(false, false,2);
    ...
    private void notifyShareAvailabilityChange(final boolean avail) {
       synchronized (mListeners) {
        ...
        if (LenovoFeatureOption.LENOVO_MASS_STORAGE_SUPPORT) {
            if (!avail) {
                /* 
                 *  There are two cases need turn off
                 *  1. is turning on UMS
    ...
    private void addVolumeLocked(StorageVolume volume) {
        ...
        //qiaoshan1 added for BLADEFHD-2887, insert new user's primary volume before the non-primary volumes.
        if (true == volume.isPrimary()){
            int i = 0;
            for (; i < mVolumes.size(); i++){
                if (false == mVolumes.get(i).isPrimary()){
                    mVolumes.add(i, volume);
                    break;
                }
            }
            if (i == mVolumes.size()){
    ...
    private void updateDefaultpath() {
        ...
        String userPath = StorageManager.getUserPath();
        String path = defaultPath;
        boolean needChange = false;
        // Priority user settings. oujf1/mengfd1 13-05-3
        if (!defaultPath.equals(userPath) && Environment.MEDIA_MOUNTED.equals(getVolumeState(userPath))) {
            StorageManager.setDefaultPath(userPath);
