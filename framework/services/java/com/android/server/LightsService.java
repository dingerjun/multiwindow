...
public class LightsService {
    ...
    public final class Light {
        ...
        private final ContentObserver mObserver = new ContentObserver(new Handler()){
            ...
            int nLED = Settings.System.getInt(mContext.getContentResolver(),Settings.System.LED_LIGHT,1);
            if(nLED == 0){
                if(mId == LIGHT_ID_BATTERY || mId == LIGHT_ID_NOTIFICATIONS)
                turnOff();
                }
            }
        };
        ...
        private Light(int id) {
            mId = id;
            //add led light observer
            mContext.getContentResolver().registerContentObserver(Settings.System.getUriFor(Settings.System.LED_LIGHT),true,mObserver);
            //end add
        ...
        private void setLightLocked(int color, int mode, int onMS, int offMS, int brightnessMode) {
            if (color != mColor || mode != mMode || onMS != mOnMS || offMS != mOffMS) {
                ...
                if(mId == LIGHT_ID_BATTERY || mId == LIGHT_ID_NOTIFICATIONS){
                    if(Settings.System.getInt(mContext.getContentResolver(),"led_light",1) != 1){
                        color = 0;
                    }else{ 
                        mColor = color;
                    }
