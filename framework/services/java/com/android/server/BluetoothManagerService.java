...
class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    private boolean checkIfCallerIsSystemUi() {
        PackageManager pm = mContext.getPackageManager();
        int uid = Binder.getCallingUid();
        String name = pm.getNameForUid(uid);
        boolean valid = false;
        if (name != null && name.contains("android.uid.systemui")) {
            valid = true;
        }

        if (DBG) Log.d(TAG, "checkIfCallerIsSystemUi: valid=" + valid + " callerName=" + name);
        return valid;
    }
