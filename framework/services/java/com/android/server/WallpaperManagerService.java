...
class WallpaperManagerService extends IWallpaperManager.Stub {
    ...
    /**Begin, lenovo-sw yuzl1, add lockscreen wallpaper feature, 2014,5,7*/
    byte[] mScreenLock = new byte[0];
    /**End, lenovo-sw yuzl1, add lockscreen wallpaper feature, 2014,5,7*/
    ...
    /**Begin, lenovo-sw yuzl1, add lockscreen wallpaper feature, 2014,5,7*/
    private static final File LOCKSCREEN_WALLPAPER_BASE_DIR = new File("/data/system/users");
    private static final String LOCKSCREEN_WALLPAPER = "lockScreenWallpaper";
    /**End, lenovo-sw yuzl1, add lockscreen wallpaper feature, 2014,5,7*/

    /**Begin, lenovo-sw yuzl1, add lockscreen wallpaper feature, 2014,5,7*/
    /**
     * Change the current lock screen wallpaper to a specific byte stream. The
     * give InputStream is copied into persistent storage and will now be used
     * as the wallpaper. Currently it must be either a JPEG or PNG image.
     *
     * @hide
     */
    public ParcelFileDescriptor setLockScreenWallpaper(String name){
        ...
        synchronized (mScreenLock) {
            ...
            try {
                try {
                	/**Begin, lenovo-sw yuzl1, add lockscreen wallpaper feature, 2014,5,14*/
                	String path = name;
                	if(path == null) {
                		path = LOCKSCREEN_WALLPAPER_BASE_DIR + "/" + userId;
                	}
                    File lockScreenWallpaperDir = new File(path);
                    File lockScreenWallpaperFile = new File(lockScreenWallpaperDir, LOCKSCREEN_WALLPAPER);
                    if (name != null && !lockScreenWallpaperFile.exists()) {
                    	return null;
                    }
                    /**End, lenovo-sw yuzl1, add lockscreen wallpaper feature, 2014,5,14*/
                    ParcelFileDescriptor pfd = ParcelFileDescriptor.open(lockScreenWallpaperFile,
