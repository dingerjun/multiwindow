...
public class UsbDeviceManager {
    ...
    //BLADEFHD-295 Begin, USB Access Mode, BLADEFHD-237,292,302
    private boolean mUsbConnectionShowFlag = false;
    private boolean mProvisioned = false;
    private boolean mBootInitiated = true; //in case Usb device manager is reboot during device run
    private static final int BOOT_INITIATED_DELAY = 3000;
    //BLADEFHD-295 end
    ...
    private final class UsbHandler extends Handler {
        ...
        private void startUsbConnection() {
          // only show when device is provisioned.
          if (!mProvisioned) {
              mProvisioned = Settings.Global.getInt(mContentResolver, Settings.Global.DEVICE_PROVISIONED, 0) != 0;
          }
          boolean isUsbAutoShow = SystemProperties.getInt("persist.sys.usbconnection.show", 1) == 1;
          if (mConnected && isUsbAutoShow && !mUsbConnectionShowFlag && mBootInitiated && mBootCompleted && mProvisioned) {
            try {
                Intent intent = new Intent();
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intent.setClassName("com.android.settings", "com.android.settings.deviceinfo.UsbConnectionActivity");
                //qiaoshan1 modifed for BLADEFHD-2884 start UsbConnectionActivity as current user
                mContext.startActivityAsUser(intent, UserHandle.CURRENT);
                mUsbConnectionShowFlag = true;
            } catch (Exception e) {
                Slog.d(TAG, "start UsbConnectionActivity error: " + e);
            }
          }
        }
        //End BLADEFHD-237
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MSG_UPDATE_STATE:
                    ...
                    // Begin BLADEFHD-237
                    if(mConnected && mBootCompleted) {
                        startUsbConnection();
                    }
                    else if (!mConnected) {
                        Intent usbConnectIntent = new Intent();
                        usbConnectIntent.setAction("android.intent.action.closeusbconnection");
                        int flags = usbConnectIntent.getFlags();
                        if((flags & Intent.FLAG_RECEIVER_REGISTERED_ONLY) != 0){
                            mContext.sendBroadcast(usbConnectIntent);
                        }
                        mUsbConnectionShowFlag = false;
                    }
                    //End BLADEFHD-237
                ...
                case MSG_BOOT_COMPLETED:
                    //BLADEFHD-295 begin
                    //When USB connected and device reboot, do not show USB Access Mode since it need some time to initiated the home screen.
                    if (mConnected) {
                        mBootInitiated = false;
                        mHandler.postDelayed(new Runnable() {
                            public void run() {
                                mBootInitiated = true;
                            }
                        }, BOOT_INITIATED_DELAY);
                    }
                    //BLADEFHD-295 end
                ...
                if (id != 0) {
                    ...
                    Intent intent = Intent.makeRestartActivityTask(
                            new ComponentName("com.android.settings",
                                    "com.android.settings.deviceinfo.UsbConnectionActivity")); //BLADEFHD-237
