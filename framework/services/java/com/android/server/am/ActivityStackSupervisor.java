...
public final class ActivityStackSupervisor {
    ...
    private int mMark = 1;//Added by ligq2 for Set Screen Effect
...
//dingej1 end.
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        /* begin, wangyan22 for smart */
        if(true){
            if((aInfo != null) && (aInfo.packageName != null) && (aInfo.name != null)) {
                Set<String> categories = intent.getCategories();
                int flags = intent.getFlags();
                int startType = 0;//SmartAPI.START_PARA_UNDEFINED;

                if ((categories == null)
                    || !categories.contains(Intent.CATEGORY_HOME)) {
                    if ((flags & (Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED)) != 0
                            && Intent.ACTION_MAIN.equals(intent.getAction())
                            && (categories != null)
                            && categories.contains(Intent.CATEGORY_LAUNCHER)) {
                        startType = 1;//SmartAPI.START_PARA_LAUNCHER;
                    }

					if(aInfo != null && aInfo.packageName != null) {
					long date = System.currentTimeMillis();
						Intent intent1 = new Intent("com.lenovo.ue.application");

						intent1.putExtra("Type", 1);
						intent1.putExtra("Package", aInfo.packageName);
						intent1.putExtra("Time", date);
						intent1.putExtra("Where", startType);

						if (ActivityManagerNative.isSystemReady()) {
							Slog.v(TAG, "send smart intent for new activity " +
                                                                aInfo.packageName+" Where "+startType);
        ...
        /* end, wangyan22 for smart */

        if (err == ActivityManager.START_SUCCESS) {
            final int userId = aInfo != null ? UserHandle.getUserId(aInfo.applicationInfo.uid) : 0;

            /*Begin Lenovo-sw piyy1 add 2014-01-24, add to support for smart side bar*/
            ComponentName componentName = intent.getComponent();
            if(componentName != null){
                String compName = componentName.toString();
                String action = intent.getAction();
                Set<String> categories = intent.getCategories();
                Slog.i(TAG,"app launched,getComponentName:" + compName + "action:" + action);
                if(action != null && action.equals(Intent.ACTION_MAIN) && categories !=null)
                {
                    for(Iterator it= categories.iterator();it.hasNext();){
                        if(it.next().equals(Intent.CATEGORY_LAUNCHER)){
                            Intent intent_applaunched = new Intent("action_smartsidebar_app_launched");
                            intent_applaunched.putExtra("package_name",componentName.getPackageName());
                            intent_applaunched.putExtra("class_name",componentName.getClassName());
                            mContext.sendBroadcast(intent_applaunched);
                        }
                    }
	        }
            }
    ...
    final ActivityRecord activityIdleInternalLocked(final IBinder token, boolean fromTimeout,
        ...
        if (allResumedActivitiesIdle()) {
            ...
            if(isCABCOn || (isPVT && isCABCNotSet)) {
                if(r != null){
                       if ("com.lenovo.scg".equals(r.packageName)
                              || "com.mt.mtxx.mtxx".equals(r.packageName)
                              || "com.lenovo.scgqc".equals(r.packageName)
                              || "com.android.gallery3d".equals(r.packageName)) {
                                 if(mMark != 0){
                                     Slog.d(TAG, "Set Screen Effect PackageName: " + r.packageName);
                                     PictureQuality.setScreenEffectCabc(0);
                                     mMark = 0;
                                 }
                       } else {
                               if(mMark != 1){
                                  Slog.d(TAG, "Set Screen Effect PackageName Out: " + r.packageName);
        ...
        /**
         * modify lenovo-sw gengse1 2014/05/09 BLADEFHDKK-82
         * Sometimes front stack is not home stack, such as GooglePinyin,
         * system will not send BOOT_COMPLTED intent. Now check mService.mBooting's value,
         * if true, system also send BOOT_COMPLETED intent.
         */
        if (isFrontStack(mHomeStack) || mService.mBooting) {
...
//dingej1 2014.05.22 begin. BLADEFHDKK-127,BLADEFHDKK-144,BLADEFHDKK-145.
/*WakeLock Manager is more strict from 4.4 version.It check calling uid and pid.
    mLaunchingActivity is initialized in systemserver process
    mLaunchingActivity.acquire() can only called in uid 1000.
    This api called from systemui is not in uid 1000 but in 10010.Force clear binder calling identity.
    All AMS other api always use clearCallingIdentity();
*/
