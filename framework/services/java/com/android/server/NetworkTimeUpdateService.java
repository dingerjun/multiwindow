...
//BEGIN Lenovo wangyq12 BLADEFHD-319 Default date/time is not correct after factory reset.
...
public class NetworkTimeUpdateService {
    ...
    //BEGIN Lenovo wangyq12 BLADEFHD-319 Default date/time is not correct after factory reset.
    private static final String DECRYPT_STATE = "unencrypted";
    private static final String CRYPT_SYS_PROPERTY = "ro.crypto.state";
    ...
    public void systemRunning() {
        ...
        //get default year in ro.build.date.utc otherthan ro.build.date,
        //since the time format of latter in different build server may varies.
        String buildDate = SystemProperties.get("ro.build.date.utc");
        if (!("".equals(buildDate))) {
            //getYear() returns (year - 1900), so add it again.
            mDefaultYear = (new Date(Long.parseLong(buildDate) * 1000)).getYear() + 1900;
        }
        ...
        if(isFirstBoot){
            ...
            String decryptState = SystemProperties.get(CRYPT_SYS_PROPERTY, "");
            Log.d(TAG, "decryptState:" + decryptState);
            if("".equals(decryptState) || DECRYPT_STATE.equals(decryptState)){
                Time today = new Time(Time.getCurrentTimezone());
                Log.d(TAG, "First boot with date:" + today + " on image released at " + buildDate  + ", " + mDefaultYear);
