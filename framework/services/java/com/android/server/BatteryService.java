...
public final class BatteryService extends Binder {
    ...
    private static final boolean LOGFILE = true;//zzf add for batterylog
...
//write rotated logs by zzf+++++++++++++++++++++++++	 
	...
	{	
		...
		if(!newfile)//compute the file size
		{
			...
			if(f1.exists()){
				try {
						...
						Slog.i(TAG,"get file size,but file not found");	
						e.printStackTrace();
					}
				try {
						FileSize = fis.available();
						fis.close();
						Slog.i(TAG,"file is:"+FileSize+"bytes");	
					} catch (IOException e) {
						// TODO Auto-generated catch block
						Slog.i(TAG,"get file size error");	
						e.printStackTrace();
					}
		...
		if(newfile){
			...
			{
			    	File mfile=files[i];
			       if(isTheSuffix(mfile.getName(), "batterylog")){
		...
		FileWriter fw = null;
		 if(newfile){		 	
			...
			destname = "/data/local/log/aplog/batterylog"+newFileTime+".txt";
		 	file = new File(destname);
			try {
				file.createNewFile();	
				FileUtils.setPermissions(file.getPath(), 00644, -1, -1);
			} catch (IOException e) {
				Slog.i(TAG,"create batterylog failed====================>");
			}
			//Log.i(TAG,"create batterylog successfully====================>"+nowTime);
			newfile = false;
		 }else{
			file = new File(destname);
		 }
		...
		if(mBatteryProps.batteryStatus >= BatteryManager.BATTERY_STATUS_UNKNOWN && mBatteryProps.batteryStatus <= BatteryManager.BATTERY_STATUS_FULL)
			myPlugType += ":" + battery_status[mBatteryProps.batteryStatus];
		
		myBatteryStatus= mBatteryProps.batteryVoltage+" "+mBatteryProps.batteryLevel+"% "+myPlugType+" "+mBatteryProps.batteryTemperature/10+"."+mBatteryProps.batteryTemperature%10+" "+(mBatteryProps.batteryCurrentNow+500)/1000;
	        try {
					fw = new FileWriter(file,true);
					fw.write(nowTime+myBatteryStatus+'\n');
					fw.close();
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
