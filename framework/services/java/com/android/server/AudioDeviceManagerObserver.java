...
public class AudioDeviceManagerObserver extends UEventObserver {
    ...
    public static final String HDMI_DEV = "AUDIO_HDMI";
    ...
    private final class BootCompletedReceiver extends BroadcastReceiver
    {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            startObserving("DEVPATH=" + hdmiDevPath);
    ...
    private String[] getDirFile(String pathString) {        
        String[] fileStrings = null;
        File file = new File(pathString);
        if (file.isDirectory()) {
            File[] fileArray = file.listFiles();
            fileStrings = new String[fileArray.length];
            if (null != fileArray && 0 != fileArray.length) {
                for (int i = 0; i < fileArray.length; i++) {
                    //System.out.println("videotest------------::"+fileArray[i].getName());
                    fileStrings[i] = pathString+"/"+fileArray[i].getName();
                }
            }
        }
        return fileStrings;
        ...
        for (int card = 0; card < MAX_AUDIO_DEVICES; card++) {
            try {
                ...
                //    String[] list = cardDir.list();
                    String[] list = getDirFile(newCard);
...
//                {
                    ...
                    if (new File("/sys/devices/virtual/switch/hdmi/state").exists()) {
                        ...
                        try {
                            ...
                            if (n > 1) {
                                hdmiplug = 0 != Integer.parseInt(new String(buf, 0, n-1));
                            }
...
//        context.registerReceiver(receiver, filter);
    ...
    public void onUEvent(UEventObserver.UEvent event) {
        ...
        try {
            ...
            if((devPath != null) && (devPath.equals(hdmiDevPath))){
                String switchName = event.get("SWITCH_NAME");
                int state = Integer.parseInt(event.get("SWITCH_STATE"));
                Log.d(TAG, "device " + switchName + ((state != 0) ? " connected" : " disconnected"));
                updateState((state != 0) ?AudioManager.AUDIO_NAME_HDMI:AudioManager.AUDIO_NAME_CODEC,
                            AUDIO_OUTPUT_TYPE, (state != 0) ? PLUG_IN : PLUG_OUT, HDMI_DEV);
            }
    ...
    public int getHdmiAvailableState(){
...
//            Log.d(TAG,"hdmi state is " + d);
            ...
            Slog.e(TAG, "Could not parse switch state for " + hdmiState + " ,fail because " + e.getMessage());
            return AudioSystem.DEVICE_STATE_UNAVAILABLE;
        }
    }
}
