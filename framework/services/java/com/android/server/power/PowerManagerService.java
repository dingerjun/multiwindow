...
//yangjq, 20130527, Add for wakelock log, START
import java.io.*;
...
//yangjq, 20130527, Add for wakelock log, END
...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    //yangjq, 20130527, Add for wakelock log
    private static final boolean LOGFILE_WAKELOCK = true;
    ...
    //yangjq, 20130527, Add for wakelock log
    private static final int MSG_ID_SAVELOG = 1;
    ...
    //yangjq, 20130527, Add for wakelock log, START
    Handler mWakeLogHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
			
            if(LOGFILE_WAKELOCK) {
                if (SystemProperties.get("init.svc.dmesglog","stopped").equals("running")) {
                    writeWKLog();
                }
                    mWakeLogHandler.sendEmptyMessageDelayed(MSG_ID_SAVELOG,15000);
            }
        }
    };
    //yangjq, 20130527, Add for wakelock log, END
    ...
    public PowerManagerService() {
        ...
        //yangjq, 20130527, Add for wakelock log, START
        mWakeLogHandler.sendEmptyMessageDelayed(MSG_ID_SAVELOG,20000);
        DeleteSleepLog();
        //yangjq, 20130527, Add for wakelock log, END
    }
    ...
    //yangjq, 20130527, Add for wakelock log, START
    public String dump_wakelocklist() {
        String pw="";
        ...
        synchronized (mLock) {
            pw+=("  mIsPowered=" + mIsPowered + "\n");
            pw+=("  mLastWakeTime=" + TimeUtils.formatUptime(mLastWakeTime) + "\n");
            pw+=("  mLastSleepTime=" + TimeUtils.formatUptime(mLastSleepTime) + "\n");
            pw+=("  mScreenBrightnessSetting=" + mScreenBrightnessSetting + "\n");

            pw+=("\n");
            pw+=("Wake Locks: size=" + mWakeLocks.size() + "\n");
            for (WakeLock wl : mWakeLocks) {
                pw+=("  " + wl + "\n");
            }
        }
...
private int TotalFileNum = 5;
private int TotalFileSize = 512000;//the largest File Size is 512K
private void writeWKLog()
{	
	List<String> PathsArray=null;
	String myS =dump_wakelocklist();	
	if(myS=="")return;
	
	PathsArray=new ArrayList<String>();
	File f=new File("/data/local/log/aplog/");
    	File[] files=f.listFiles();
	FileInputStream fis = null;
	int FileSize = 0;
	if(!newfile)//compute the file size
	{		
		File f1 = new File(destname);
		if(f1.exists()){
			try {
					fis=new FileInputStream(new File(destname));
				} catch (FileNotFoundException e) {
					// TODO Auto-generated catch block					
					Log.i(TAG,"get file size,but file not found");	
					e.printStackTrace();
				}
			try {
					FileSize = fis.available();
					fis.close();
					Log.i(TAG,"file is:"+FileSize+"bytes");	
				} catch (IOException e) {
					// TODO Auto-generated catch block
					Log.i(TAG,"get file size error");	
					e.printStackTrace();
				}	
	...
	if(newfile){
		for(int i=0;files != null && i<files.length;i++)//put the filePaths in an ArrayList
		{
		    	File mfile=files[i];
		       if(isTheSuffix(mfile.getName(), "wklog")){
		    		PathsArray.add(mfile.getPath());
		    	}    		
		}
	}
	if((newfile&&PathsArray.size()>= TotalFileNum)||(!newfile&&FileSize>= TotalFileSize))//while the number of batterylog is 5 or the File Size is 512k,we delet the first one
	{
		Collections.sort(PathsArray);//sort the filePaths and then delete the files which is more than  TotalFileNum
		Log.i(TAG,"delete wk log");	
		for(int n=0;n<=PathsArray.size()-TotalFileNum;n++){
			DeleteFile(PathsArray,n);
		}
		newfile = true;
	}	
	...
	FileWriter fw = null;
	 if(newfile){			
		...
		newFileTime = df1.format(dt);
		destname = "/data/local/log/aplog/wklog"+newFileTime+".txt";
		file = new File(destname);
		try {
			file.createNewFile();	
			FileUtils.setPermissions(file.getPath(),0644,-1,-1);
		} catch (IOException e) {
			//Log.i(TAG,"create wklog failed====================>");
		}
		//Log.i(TAG,"create wklog successfully====================>"+nowTime);
		newfile = false;
	 }else{
		file = new File(destname);
	 }
...
private boolean isTheSuffix(String file, String suffix) {
    		for (int i = 0; i < suffix.length(); i++) {
    			if (file.charAt(i) != suffix.charAt(i)) {
    				return false;
    			}
    		}
    		return true;
	...
	private void DeleteFile(List<String> PathsArray,int index){
    		final File mfile;
    		mfile=new File(PathsArray.get(index));
       	mfile.delete();
    	}

	private boolean StartFlg = true;
	private void DeleteSleepLog()//every time we start the machine,we check the number of sleeplog files,and delete the files that more than TotalFileNum
	{
		if(StartFlg){
			List<String> PathsArray=null;
			PathsArray=new ArrayList<String>();
			...
			{
			    	File mfile=files[i];
			       if(isTheSuffix(mfile.getName(), "sleeplog")){
			    		PathsArray.add(mfile.getPath());
			    	}    		
			}
			if(PathsArray.size()>= TotalFileNum)//while the number of sleeplog is 5 ,we delet the first one
			{
				Collections.sort(PathsArray);//sort the filePaths and then delete the files which is more than  TotalFileNum
				Log.i(TAG,"delete Sleep log");	
				for(int n=0;n<=PathsArray.size()-TotalFileNum;n++){
					DeleteFile(PathsArray,n);
				}				
			}
